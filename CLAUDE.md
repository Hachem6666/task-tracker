...
- **PATCH null-tags rule**: `tags` cannot be explicitly set to `null` on `PATCH /tasks/{task_id}` — the `TaskUpdate` validator in `app/models.py` rejects it with `422`, mirroring the null-title rule. Omitting `tags` from the PATCH body leaves existing tags unchanged; only an explicit `null` is rejected.
